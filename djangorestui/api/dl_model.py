...
def get_predicted_class_prob(model_obj, val_data, test_data=None):
    val_preds = pd.DataFrame(
        model_obj.predict_proba(val_data)
    )  # .apply(lambda x : x.max(), axis=1)
    if test_data is not None:
        test_preds = pd.DataFrame(
            model_obj.predict_proba(test_data)
        )  # .apply(lambda x : x.max(), axis=1)
        return (val_preds, test_preds)
    return val_preds
...
def extract_features(y, sr):
    ...
    We are extracting the Chroma features, spectral centroid, spectral bandwidth, spectral roll off,
     zero crossing rate and the mfccc's"""
...
def create_model_basic_nn():
    ...
    model_basic_nn.compile(
        loss="categorical_crossentropy", optimizer="adam", metrics=["acc", recall]
    )
...
def create_model_rnn():
    ...
    model_basic_rnn.add(SimpleRNN(256, input_shape=(1, input_shape)))
...
def return_prediction():
    ...
    if input_params:
        ...
        try:
            ...
            print("return_prediction failed with exception : {}".format(e))
