...
from .serializers import SongsInputSerializer
...
def simple_upload(request):
    if request.method == "POST" and request.FILES["myfile"]:
        ...
        return render(
            request, "main/sample_upload.html", {"uploaded_file_url": uploaded_file_url}
        )
...
def uploaded_files(request):
    ...
    return render(
        ...
        context={"songs": file_names},
    )


def blank_page(request):
    if request.GET.get("run_model"):
        return render(request=request, template_name="main/blank.html")
...
def style_transfer_code_page(request):
    return render(request=request, template_name="main/Style_Transfer.html")
...
def select_song(request):
    if request.GET.get("select_song"):
        ...
        return render(
            ...
            context={
                "songs": request.GET.get("textbox_1"),
                # "models":models
            },
        )
...
def run_model(request):
    if request.GET.get("run_model"):
        ...
        table = {"model_name": model_name, "song_name": song_name}
        print("table", table)
...
def return_plot(request):
    ...
    if (request.GET.get("waveplot")) or (request.GET.get("spectogram")):
        ...
        return render(
            ...
            context={"plot_name": request.GET.get("song_box")},
        )
